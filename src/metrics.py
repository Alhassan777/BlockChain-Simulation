...
class BlockMetric:
    ...
    miner_id: str
    creation_time: float
    propagation_times: Dict[str, float] = field(
        default_factory=dict
    )  # node_id -> time received
...
class MetricsCollector:
    ...
    def record_transaction_submitted(self, tx_hash: str, node_id: str) -> None:
        ...
        with self._lock:
            ...
            metric = TransactionMetric(tx_hash=tx_hash, creation_time=current_time)
    ...
    def record_transaction_confirmed(
        self, tx_hash: str, block_index: int, confirmation_time: Optional[float] = None
    ) -> None:
        """Record when a transaction is confirmed in a block."""
        with self._lock:
            if tx_hash in self.transactions:
                self.transactions[tx_hash].confirmation_time = (
                    confirmation_time or time.time()
                )
    ...
    def get_block_propagation_delay(
        self, block_hash: str
    ...
    # ==================== Unlocked Helper Methods (for use within locked context) ====================

    def _calculate_tps_unlocked(self) -> float:
        """Calculate TPS without acquiring lock (must be called within locked context)."""
        current_time = time.time()
        elapsed = current_time - self.last_tps_calculation
        ...
        tps = self.tx_count_since_last_tps / elapsed if elapsed > 0 else 0.0
        return tps

    def _get_average_tps_unlocked(self) -> float:
        """Get average TPS without acquiring lock."""
        if not self.tps_history:
            return 0.0
        return statistics.mean(self.tps_history)

    def _get_average_block_time_unlocked(self) -> float:
        """Get average block time without acquiring lock."""
        if not self.block_time_history:
            return 0.0
        return statistics.mean(self.block_time_history)

    def _get_average_block_propagation_delay_unlocked(self) -> float:
        """Get average propagation delay without acquiring lock."""
        all_delays = []
    ...
    def _get_orphan_rate_unlocked(self) -> float:
        """Get orphan rate without acquiring lock."""
        if self.total_blocks_mined == 0:
            return 0.0
        return (self.total_orphans / self.total_blocks_mined) * 100

    def _get_average_confirmation_latency_unlocked(self) -> float:
        """Get average confirmation latency without acquiring lock."""
        latencies = []
    ...
    def _get_average_mining_time_unlocked(self) -> float:
        """Get average mining time without acquiring lock."""
        mining_times = []
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._lock.acquire(timeout=0.5):
                # Return default values if can't get lock
                return {
                    ...
                    "average_propagation_delay": 0.0,
                    "orphan_rate_percent": 0.0,
                    "average_confirmation_latency": 0.0,
                    "average_mining_time": 0.0,
                    "blocks_per_minute": (
                        (self.total_blocks_mined / runtime) * 60 if runtime > 0 else 0
                    ),
                }

            try:
                # Calculate all metrics while holding lock
                current_tps = self._calculate_tps_unlocked()
                average_tps = self._get_average_tps_unlocked()
                average_block_time = self._get_average_block_time_unlocked()
                average_propagation = (
                    self._get_average_block_propagation_delay_unlocked()
                )
                orphan_rate = self._get_orphan_rate_unlocked()
                avg_confirmation = self._get_average_confirmation_latency_unlocked()
                avg_mining_time = self._get_average_mining_time_unlocked()
                ...
                return {
                    ...
                    "average_propagation_delay": average_propagation,
                    "orphan_rate_percent": orphan_rate,
                    "average_confirmation_latency": avg_confirmation,
    ...
    def get_recent_blocks_summary(self, count: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                for block_hash in reversed(blocks_to_show):
                    if block_hash in self.blocks:
                        ...
                        # Calculate propagation delay inline (already have lock)
                        prop_delay = None
                        if block_hash in self.blocks:
                            creation_time = metric.creation_time
                            delays = [
                        ...
                        summaries.append(
                            {
                                ...
                                "propagation_delay_mean": (
                                    round(prop_delay["mean"], 3) if prop_delay else None
                                ),
                                "nodes_reached": (
                                    prop_delay["nodes_reached"] if prop_delay else 1
                                ),
            ...
            self.logger.error(f"Error getting recent blocks summary: {e}")
            return []
    ...
    def print_summary(self) -> None:
        ...
        print(
            f"  Avg confirmation latency: {summary['average_confirmation_latency']:.2f}s"
        )
        print(f"\n🌐 Network:")
        print(
            f"  Avg propagation delay: {summary['average_propagation_delay']*1000:.1f}ms"
        )

...
class Blockchain:
    ...
    def add_block(self, block: Block) -> Tuple[bool, str]:
        ...
        if block.index != len(self.chain):
            return (
                False,
                f"Invalid index: expected {len(self.chain)}, got {block.index}",
            )
    ...
    def replace_chain(self, new_chain: List[Block]) -> Tuple[bool, str]:
        ...
        # Track orphaned blocks before replacing chain
        from src.metrics import get_metrics_collector

        metrics = get_metrics_collector()

        # Create set of block hashes in the new chain for fast lookup
        new_chain_hashes = {block.hash for block in new_chain}

        # Mark blocks that will become orphans
        orphan_count = 0
        for block in self.chain:
            # Skip genesis block
            if block.index == 0:
                continue
            # If block is not in new chain, it's being orphaned
            if block.hash not in new_chain_hashes:
                metrics.record_block_orphaned(block.hash)
                orphan_count += 1
        ...
        if orphan_count > 0:
            ...
            logger.info(
                f"Marked {orphan_count} block(s) as orphaned during chain replacement"
            )

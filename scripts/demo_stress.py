...
async def create_nodes(num_nodes: int, genesis: Block, difficulty: int = 2):
    ...
    for node in nodes:
        await node.start()
        await asyncio.sleep(0.05)  # Small delay between starts
...
async def connect_mesh(nodes):
    ...
    for i, node1 in enumerate(nodes):
        for node2 in nodes[i+1:]:
            ...
            connection_count += 1
            await asyncio.sleep(0.02)  # Small delay between connections
...
async def connect_ring_with_shortcuts(nodes, shortcut_count: int = 5):
    ...
    while shortcuts_added < shortcut_count:
        ...
        if nodes[j].node_id not in nodes[i].network.peers:
            await nodes[i].connect_to_peer(
                nodes[j].node_id,
                nodes[j].network.host,
                nodes[j].network.port
            )
            shortcuts_added += 1
            await asyncio.sleep(0.02)
...
async def bootstrap_economy(nodes, blocks_per_node: int = 1):
    ...
    for round_num in range(blocks_per_node):
        for node in nodes:
            ...
            if block:
                print(f"  Round {round_num + 1}: {node.node_id} mined block #{block.index}")
            await asyncio.sleep(0.5)  # Let block propagate
...
async def main():
    ...
    parser.add_argument('--difficulty', type=int, default=2, help='Mining difficulty (default: 2)')
    parser.add_argument('--transactions', type=int, default=50, help='Number of transactions (default: 50)')
    parser.add_argument('--mining-rounds', type=int, default=5, help='Mining rounds (default: 5)')

...
async def main():
    ...
    print(
        f"  ✓ TX in partition A: {nodes[0].miner_address[:8]}... → {nodes[1].miner_address[:8]}..."
    )
    ...
    print(
        f"  ✓ TX in partition B: {nodes[2].miner_address[:8]}... → {nodes[3].miner_address[:8]}..."
    )
    ...
    for node in nodes:
        ...
        print(
            f"  {node.node_id}: {node.blockchain.get_chain_length()} blocks, tip={tip}..."
        )

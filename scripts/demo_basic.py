...
async def main():
    ...
    for i in range(4):
        ...
        nodes.append(node)
        print(f"  - Node {i} created on port {base_port + i}")
    ...
    for round_num in range(2):  # 2 rounds
        for node in nodes:
            ...
            if block:
                ...
                print(f"  ✓ {node.node_id} balance: {node.blockchain.get_balance(node.miner_address):.2f} coins")
            else:
    ...
    for node in nodes:
        print(f"  {node.node_id}: {node.blockchain.get_chain_length()} blocks, balance={node.blockchain.get_balance(node.miner_address):.2f} coins")
